         ...
         if(!isNaN(feedback)){
            ...
            let lookupData=await lookupAppointmentIdAndStage(messageId); // if messageid is not known throws an error. 
         	...
         	if(currentStage==1){
            	...
            	//send thank you sms
	        	message="Please enter 1 for yes or 2 for no if you receive <Explanation of service> from DCNET";
	            	currentStage=currentStage+1;
	            	isConversationFinished=false;
	           	await updateDynamoDbFeedbackScore1 (appointmentId, feedback);
	                        }
            else if(currentStage==2){
                //update the feedback score
            	//send thank you sms
                message ="Please enter 1 for yes or 2 for no if you receive <frequency of service>";
                	currentStage=currentStage+1;
	            	isConversationFinished=false;
	           	await updateDynamoDbFeedbackScore2 (appointmentId, feedback);
	            
            }
            else if(currentStage==3){
                //update the feedback score
            	//send thank you sms
	            message ="Please enter 1 for yes or 2 for no if you have any concerns about the services you are receiving from DCNET";
	            	currentStage=currentStage+1;
	            	isConversationFinished=false;
	           await updateDynamoDbFeedbackScore3 (appointmentId, feedback);
	            
            }
             else if(currentStage==4){
                //send thank you sms
	           await updateDynamoDbFeedbackScore4 (appointmentId, feedback);
...
async function updateDynamoDbFeedbackScore1 (appointmentId, feedbackScore){
...
async function updateDynamoDbFeedbackScore2 (appointmentId, feedbackScore){
	  const strAppintmentId=appointmentId.toString();
	  let params = {
	      TableName:'feedbacks',
	      Key:{
...
async function updateDynamoDbFeedbackScore3 (appointmentId, feedbackScore){
	  const strAppintmentId=appointmentId.toString();
	  let params = {
	      TableName:'feedbacks',
	      Key:{
...
async function updateDynamoDbFeedbackScore4 (appointmentId, feedbackScore){
	  const strAppintmentId=appointmentId.toString();
	  let params = {
	      TableName:'feedbacks',
	      Key:{
